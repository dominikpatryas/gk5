...
// METODA KORKOCIAGU
    public void Korkociag(GL2 gl2, int promien, int liczbaObrotow)
    {
// CZYSZCZENIE 
        gl2.glClear(GL2.GL_COLOR_BUFFER_BIT);

        int wierzcholki = liczbaObrotow*1000;
        double inkrementacjaKata = liczbaObrotow * Math.PI / wierzcholki;
        float wysokosc=-8;
// ROZPOCZECIE RYSOWANIA WIELOKATA
...
// OKRESLENIE WSPOLRZEDNYCH
            gl2.glVertex3f(x, wysokosc, z);
            kat = i * inkrementacjaKata;
            wysokosc+=liczbaObrotow/30000f;
        }
// KONIEC RYSOWANIA
        gl2.glEnd();
// USTAWIENIE
...
// METODA ODPOWIEDZIALNA ZA TROJKATY
    ...
    {
// POCZATEK RYSOWANIA TROJKATA 
        gl2.glBegin(gl2.GL_TRIANGLE_FAN);
        int n = 11;
// OKRESLENIE WSPOLRZEDNYCH
        gl2.glVertex3f(0,4,0);
        gl2.glVertex3f(1,0,0);
// PETLA WYKONUJACA SIE N RAZY CZYLI TYLE CO MA WARIANT 11
        for ( int i = 0; i<n;i++)
        {
        	// KOLOR
            gl2.glColor3f(0.0f,0.0f,1.0f);
            gl2.glVertex3f((float) (Math.cos(i*2*Math.PI/n)*wielkosc), 0, (float) (Math.sin(i*2*Math.PI/n)));
         // KOLOR
            gl2.glColor3f(0.0f, 1.0f,0.0f);
            gl2.glVertex3f((float) (Math.cos((i+1)*2*Math.PI/n)*wielkosc), 0, (float) (Math.sin((i+1)*2*Math.PI/n)*
            		wielkosc));
        }
// KONIEC RYSOWANIA TROJKATA
        gl2.glEnd();
// POCZATEK RYSOWANIA WIELOKATU
        gl2.glBegin(gl2.GL_POLYGON);
// USTAWIENIE POCZATKOWYCH WSPOLRZEDNYCH WIELOKATU
        gl2.glVertex3f(0,0,0);
        for (int i=0; i<n;i++)
        {
        	// KOLOR
            gl2.glColor3f(1.0f,0.0f,1.0f);
            // USTALENIE WSPOLRZEDNYCH KTORE SA INKREMENTOWANE Z KAZDA PETLA
            gl2.glVertex3f((float) (Math.cos(i*2*Math.PI/n)*wielkosc), 0, (float) (Math.sin(i*2*Math.PI/n)*wielkosc));
         // KOLOR
            gl2.glColor3f(0.0f,0.0f,1.0f);
         // USTALENIE WSPOLRZEDNYCH KTORE SA INKREMENTOWANE Z KAZDA PETLA
            gl2.glVertex3f((float) (Math.cos((i+1)*2*Math.PI/n)*wielkosc), 0, (float) (Math.sin((i+1)*2*Math.PI/n)*
